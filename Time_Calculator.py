...
def add_time(start, duration, day_started = ""):
    ...
    global Result_Gui
    Result_Gui.destroy()
    Result_Gui = Label(root,text=Result, bg="Black", fg="White",font=("Bold",32),pady=80)
    Result_Gui.grid(row=3, column=1)
    return Result
Instruction_left = Label(root, text="Write the starting time below:",font=("Bold",21))
Instruction_left.grid(row=0,column=0)
thing_that_makes_all_more_separated = Label(root,padx=250,pady=100, bg="black")
thing_that_makes_all_more_separated.grid(row=0,column=1)
Instruction_right = Label(root, text="Write the hours to add below:",font=("Bold",21))
Instruction_right.grid(row=0,column=2)
Starting_time = Entry(root, bg = "grey", fg="white", font=("Bold",23))
# Starting_time.insert(0, "Starting_time: ")
Starting_time.grid(row=1,column=0,padx=50)
Hours_to_add = Entry(root,bg = "grey", fg= "white",font=("Bold",23))
# Hours_to_add.insert(0, "Hours_to_add: ")
Hours_to_add.grid(row=1,column=2,padx=50)
Calculate = Button(root, text="Calculate", padx=120, bg = "#36454F"
                    , fg= "white",pady=42,font=("Bold",33),
                    command=lambda: add_time(Starting_time.get(), Hours_to_add.get()))
